...
红黑树的删除就不再详细介绍了，其实本质上原理和红黑树的插入后的平衡调整是一样的，基本就是通过变色和旋转来调整红黑树的结构以期望再次得到新平衡。

终于把红黑树的插入操作部分写完了，用了很多示意图，这篇文章是在之前分享的 ppt 上再整理出来，我觉得自己应该算是把插入的操作讲明白了，整理这篇文章前前后后用了近一周左右，因为平时上班，基本上只有周末有时间才有时间整理，如有问题请留言讨论。
